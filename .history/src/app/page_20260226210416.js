...
export default function Home() {
  ...
  // View transform: how image is drawn inside the fixed canvas viewport
  const viewRef = useRef({ scale: 1, offX: 0, offY: 0, drawW: 0, drawH: 0 });
  ...
  // Draft item (in-progress drawing)
  const [draft, setDraft] = useState(null);

  // Mouse drag state (for drag-based tools)
  const [drag, setDrag] = useState({ active: false });

  // Optional calibration for panel
  const [pxPerCm, setPxPerCm] = useState(100);
  ...
  function angleDeg(A, V, B) {
    // angle at V between vectors VA and VB
  ...
  // Mouse (canvas CSS pixels) -> Image pixels
  function canvasToImagePoint(e) {
    const canvas = canvasRef.current;
    const rect = canvas.getBoundingClientRect();
  ...
  // Image pixels -> Canvas CSS pixels
  ...
  function drawAll() {
    ...
    // Fit image into fixed canvas (contain)
    const scale = Math.min(
      rect.width / imgObj.width,
      rect.height / imgObj.height,
    );
  ...
  function drawItem(ctx, it, isDraft = false) {
    ...
    // label bubble (measurement digit)
    function drawMeasurementLabel(text, anchorA, anchorB) {
      ...
      if (labelPos === "middle") {
        ...
        // above: perpendicular offset from middle
        const mid = {
    ...
    if (it.type === "line" && it.A && it.B) {
      ...
      // measurement text (you can change later to cm/mm)
    ...
    if (it.type === "angle" && it.A && it.V && it.B) {
      ...
      // show angle digit near vertex
      if (showLabel) {
        const deg = angleDeg(it.A, it.V, it.B);
        drawSmallLabel(ctx, V.x + 18, V.y - 18, `${deg.toFixed(1)}°`, color);
      }
    }
  ...
  // ---------- LOAD IMAGE (used by file input + drag & drop) ----------
    ...
    <main className={styles.main}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            Drag & drop • Premium lines • Tools toolbar
          </p>
        </div>
        <div className={styles.badge}>
          Tool: <b>{tool}</b>
      ...
      <div className={styles.panel}>
        ...
        <div className={styles.panelRow}>
          <span className={styles.k}>How to use</span>
          <span className={styles.v}>
            Line/Angle = click points • Rect/Circle/Draw = click+drag • Drop
            image on viewer
          </span>
        </div>
      </div>
