...
export default function Home() {
  ...
  function getDeleteZone(viewW) {
    const w = 120;
    const h = 42;
    const pad = 14;
    return { x: viewW - w - pad, y: pad, w, h };
  }

  function isInDeleteZone(canvasP, viewW) {
    const z = getDeleteZone(viewW);
    return (
      canvasP.x >= z.x &&
      canvasP.x <= z.x + z.w &&
      canvasP.y >= z.y &&
  ...
  function drawAll() {
    ...
    // Delete button: drag an existing shape here to remove it
    const zone = getDeleteZone(rect.width);
    const isDraggingShape = drag.type === "MOVE" || drag.type === "HANDLE";
    ctx.save();
    roundRect(ctx, zone.x, zone.y, zone.w, zone.h, 10);
    ctx.fillStyle = deleteHot
      ? "rgba(239, 68, 68, 0.85)"
      : "rgba(15, 23, 42, 0.82)";
    ctx.fill();
    ctx.lineWidth = 1.5;
    ctx.strokeStyle = deleteHot
    ...
    ctx.textBaseline = "middle";
    ctx.fillText(
      isDraggingShape ? "Drop to Delete" : "Delete",
      zone.x + zone.w / 2,
      zone.y + zone.h / 2,
    );
    ctx.restore();
  ...
  function handleMouseMove(e) {
    ...
    let p = clampToImage(canvasToImagePoint(e));
    const cp = eventToCanvasPoint(e);
    const viewW = canvasRef.current.getBoundingClientRect().width;

    // drag handle
    if (drag.type === "HANDLE") {
      setDeleteHot(isInDeleteZone(cp, viewW));
      setItems((prev) =>
        prev.map((it, i) => {
